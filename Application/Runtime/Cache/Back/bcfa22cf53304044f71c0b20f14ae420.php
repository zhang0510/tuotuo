								...
								<div class="tab-pane active">
									...
									<fieldset>
										<div class="l_uit">
											...
											<form method="get" action="/Back/Financeyjh/getOrderList">
											...
											<div class="l_table">
												<table>
													...
													<?php if(is_array($list)): foreach($list as $key=>$vo): ?><tr class="tr2">
															<td class="td3"><input type="checkbox" class="box" value="<?php echo ($vo["order_code"]); ?>" onclick="sum(this)"></td>
															...
															<td><?php echo ($vo['order_yun_price']); ?></td>
															
															<td><?php echo ($vo['order_info_bxd']/100); ?></td>
															
															<td><?php echo ($vo['bill_price']/100); ?></td>
															<td><?php echo ($vo['order_info_zj']/100); ?></td>
															<td class="td_d2"><?php echo ($vo['income']); ?></td>
															<td class="td_d2">
															<?php if($vo['fin_is_yun'] == 'Y'): ?><input type="checkbox" checked="checked" class="check" data-class="n<?php echo ($vo['order_id']); ?>" data-id="<?php echo ($vo['order_id']); ?>"/>
															<?php else: ?>
															    <input type="checkbox" class="check" data-class="n<?php echo ($vo['order_id']); ?>" data-id="<?php echo ($vo['order_id']); ?>"/><?php endif; ?>
															</td>
															<td class="n<?php echo ($vo['order_id']); ?>"><?php echo ($vo['fin_yun']); ?></td>
...
$(".check").click(function(){
	...
	$(".btn-default22").click(function(){
		...
		if(number==''){
			...
			$.post("/Back/Financeyjh/upOrderStatus",{table:'order_assistant',status:'fin_is_yun',number:number,liuName:'fin_yun',idName:'order_id',id:id},function(data){
				 if(data.flag){
					 $("#ggg1").hide();
					 $("#bg").hide();
