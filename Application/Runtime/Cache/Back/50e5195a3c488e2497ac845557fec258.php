								...
								<div class="tab-pane active">
									...
									<fieldset>
										<div class="l_uit">
											...
											<form method="get" action="/Back/Financeyjh/getFaList">
											...
											<div class="l_table">
												<table>
													...
													<?php if(is_array($list)): foreach($list as $key=>$vo): ?><tr class="tr2">
															...
															<td class="td_d2">
															<?php if($vo['fin_is_fa'] == 'Y'): ?><input type="checkbox" class="check" checked="checked" data-class="n<?php echo ($vo['order_id']); ?>" data-id="<?php echo ($vo['order_id']); ?>"/>
															<?php else: ?>
															       <input type="checkbox" class="check" data-class="n<?php echo ($vo['order_id']); ?>" data-id="<?php echo ($vo['order_id']); ?>"/><?php endif; ?>
															</td>
															<td class="n<?php echo ($vo['order_id']); ?>"><?php echo ($vo['fin_fa']); ?></td>
...
$(".check").click(function(){
	...
	$(".btn-default22").click(function(){
		...
		if(number==''){
			...
			$.post("/Back/Financeyjh/changeStatus",{table:'order_assistant',status:'fin_is_fa',number:number,liuName:'fin_fa',idName:'order_id',id:id},function(data){
				 if(data.flag){
