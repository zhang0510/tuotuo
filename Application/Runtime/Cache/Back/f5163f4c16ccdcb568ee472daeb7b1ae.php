								...
								<div class="tab-pane active">
									...
									<fieldset>
										<div class="l_uit">
											...
											<form method="get" action="/Back/Financeyjh/getYunList">
											...
											<div class="l_table">
												<table>
													...
													<?php if(is_array($list)): foreach($list as $key=>$vo): ?><tr class="tr2">
															<td class="td3"><input type="checkbox" class="box" names="<?php echo ($vo['order_info_zj']/100); ?>" value="<?php echo ($vo["yd_code"]); ?>" onclick="sum(this)"></td>
															<td><?php echo ($vo["yd_time"]); ?></td>
															<td><?php echo ($vo["yd_code"]); ?></td>
															<td><?php echo ($vo["order_code"]); ?></td>
															<td><?php echo ($vo["order_info_type"]); ?></td>
															<td><?php echo ($vo["order_info_carmark"]); ?></td>
															<td><?php echo ($vo['yd_type']); ?></td>
															<td><?php echo ($vo['yd_name']); ?></td>
															<td><?php echo ($vo["pay_name"]); ?></td>
															<td><?php echo ($vo['yd_line']); ?></td>
															<td><?php echo ($vo['yd_price']/100); ?>元</td>
															<td class="td_d2">
															<?php if($vo['yd_is_pay'] == 'Y'): ?><input type="checkbox" class="check" checked="checked" data-class="n<?php echo ($vo['yd_id']); ?>" data-id="<?php echo ($vo['yd_id']); ?>"/>
															<?php else: ?>
															    <input type="checkbox" class="check" data-class="n<?php echo ($vo['yd_id']); ?>" data-id="<?php echo ($vo['yd_id']); ?>"/><?php endif; ?>
															</td>
															<td class="n<?php echo ($vo['yd_id']); ?>"><?php echo ($vo['yd_liu']); ?></td>
...
$(".check").click(function(){
	...
	$(".btn-default22").click(function(){
		...
		if(number==''){
			...
			$.post("/Back/Financeyjh/changeStatus",{table:'yundan',status:'yd_status',number:number,liuName:'yd_liu',idName:'yd_id',id:id},function(data){
				 if(data.flag){
...
function daochu(){
	var selid = '';
		  ...
		  if(selid == ""){
			...
			window.location.href="/Back/Financeyjh/daochu_yun/code/"+selid;
		  }
