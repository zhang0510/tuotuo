 ...
 <script>
												...
												<div style="float:left;" class="controls clec_t">
													<div class="dropdown">
													     <div class="pay1">
															...
															<dd>
																<input type="text" placeholder="请选择区" id="county" value="<?php echo ($tval["area_name"]); ?>"/>
														...
														<input type="hidden" value="<?php echo ($tval["cid"]); ?>" id="cou"/>
														<script>
									...
									<div class="l_table">
										<table>
											...
											<?php if(is_array($list)): $k = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><tr class="tr2">
													...
													<td><?php echo ($vo["ti_driver_price"]); ?></td>
													<td><?php echo ($vo["ti_platelets_price"]); ?></td>
													<td>
														<a class="see4" data-code="<?php echo ($vo["ti_code"]); ?>">删除</a>
														<a data-toggle="modal" data-target="#myModa2" 
															 code="<?php echo ($vo["ti_code"]); ?>"
															 star="<?php echo ($vo["ti_star"]); ?>"
															 end="<?php echo ($vo["ti_end"]); ?>"
															 tp-price="<?php echo ($vo["ti_platelets_price"]); ?>"
															 td-price="<?php echo ($vo["ti_driver_price"]); ?>"
															 psname="<?php echo ($vo["p_s_name"]); ?>"
															 csname="<?php echo ($vo["c_s_name"]); ?>"
															 cosname="<?php echo ($vo["co_s_name"]); ?>"
									...
									<div class="l_fenye">
										<div class="sad">
											...
											<a href="/Back/Product/daochu_star/pro/<?php echo ($pval['pid']); ?>/city/<?php echo ($cval['cid']); ?>/cou/<?php echo ($tval['cid']); ?>"><button type="button" class="btn btn-default6">导出</button></a>
										</div>
										<?php echo ($page); ?>
									</div>
								</div>
...
<div class="tan11">
<h2>导入数据</h2>
<form action="/Back/Product/daoru_star" method="post" enctype="multipart/form-data">
<div class="dw4">
...
<script>
	...
	$('#sub').click(function(event) {
		...
		$.post('/Back/Product/staradd', {sprovince:sprovince,scity:scity,scounty:scounty,eprovince:eprovince,ecity:ecity,ti_driver_price:ti_driver_price,ti_platelets_price:ti_platelets_price,ti_code:ti_code}, function(data) {
			if(data['status'] == 'Y'){
                $.anewAlert(data['info']);
    ...
    $('#btn').click(function(event) {
        ...
        window.location.href="/Back/Product/star/provincea/"+provincea+"/city/"+city+"/county/"+county;
    });
    //删除    
    $('.see4').click(function(event) {
        var code = $(this).attr('data-code');
        if (confirm('确定删除此信息吗？')) {
            $.post('/Back/Product/stardel', {code: code}, function(data) {
                alert(data);
                window.location.reload();
            },'json');
        }
