...
<script>
	function del(obj){
		...
		$.post('/Back/Logistics/driverDel',{'id':id},function(data){
			if(data.sign=='Y'){
				layer.msg(data.con);
				...
				<div class="widget">
					...
					<div class="tabbable">
						<ul class="nav nav-tabs" id="tabnav">
							<li>
								<a href="/Back/Logistics/companyShow">物流公司</a>
							</li>
							<li class="active">
											...
											<div class="control-group">
												<div style="float:left;" class="controls clec_t">
													<div class="v_pb">
														...
														<input type="text" placeholder="请输入司机名称" name="name" value="<?php echo ($name); ?>" class="i_n text2 width360">
														<button class="btn btn-default5" type="submit">搜索</button>
													
													</div>
												</div>
												<!-- /controls -->
												<a href="/Back/Logistics/driverAdd"><button class="red btn btn-default5 fr" type="button">添加司机</button></a>
											</div>
											<script>
												$(function(){
											...
											<div class="l_table">
												<table>
													...
													<?php if(is_array($list)): foreach($list as $key=>$vo): ?><tr class="tr2">
															<td><?php echo ($vo["car_sheng_name"]); ?></td>
															<td><?php echo ($vo["car_shi_name"]); ?></td>
															<td><?php echo ($vo["car_name"]); ?>|<?php echo ($vo["car_tel"]); ?>|<?php echo ($vo["car_identity"]); ?></td>
															<td>
																<?php if(($vo["car_he_code"] != '')): echo ($vo["car_he_code"]); ?>
																<?php else: ?>
																	--<?php endif; ?>
															</td>
															<!-- <td>定位查看</td> -->
															<td>
																<a class="see1" href="/Back/Logistics/driverAdd/id/<?php echo ($vo["car_id"]); ?>">修改</a>
																<a class="see2" href="javascript:;" name="<?php echo ($vo["car_id"]); ?>" onclick="del(this);">删除</a>
															</td>
