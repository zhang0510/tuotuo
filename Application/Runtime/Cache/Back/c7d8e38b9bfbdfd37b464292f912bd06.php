									...
									<div class="l_uit">
											...
											<div style="float:left;" class="controls clec_t">
												<div class="dropdown">
												     <div class="pay1">
															...
															<dd>
																<input type="text" placeholder="请选择区" id="county" value="<?php echo ($coval); ?>"/>
														...
														<input type="hidden" value="<?php echo ($cou); ?>" id="cou"/>
														<script>
										...
										<div class="l_table">
											<table>
												...
												<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="tr2">
													...
													<td><?php echo ($vo["co_e_name"]); ?></td>
													<td><?php echo ($vo["sm_driver_price"]); ?></td>
													<td><?php echo ($vo["sm_platelets_price"]); ?></td>
													<td>
														<a class="see4" data-code="<?php echo ($vo["sm_code"]); ?>">删除</a>
														<a data-toggle="modal" data-target="#myModa2"
														 code="<?php echo ($vo["sm_code"]); ?>"
															 star="<?php echo ($vo["sm_star"]); ?>"
															 end="<?php echo ($vo["sm_end"]); ?>"
															 sd-price="<?php echo ($vo["sm_driver_price"]); ?>"
															 sp-price="<?php echo ($vo["sm_platelets_price"]); ?>"
															 psname="<?php echo ($vo["p_s_name"]); ?>"
															 csname="<?php echo ($vo["c_s_name"]); ?>"
															 coename="<?php echo ($vo["co_e_name"]); ?>"
										...
										<div class="l_fenye">
											<div class="sad">
												...
												<a href="/Back/Product/daochu_Sm/pro/<?php echo ($pro); ?>/city/<?php echo ($city); ?>/cou/<?php echo ($cou); ?>"><button type="button" class="btn btn-default6">导出</button></a>
											</div>
											<?php echo ($page); ?>
										</div>
									</div>
...
<div id="myModa2" style="display:none; z-index:9999" class="tan1">
	<div class="tan11">
		...
		<div class="dw2">
		<!-- <form action="/Back/Product/visitendadd" method="post"> -->
			<dl>
				<dt>
...
<div class="tan11">
<h2>导入数据</h2>
<form action="/Back/Product/daoru_Sm" method="post" enctype="multipart/form-data">
<div class="dw4">
...
<script>
	...
	$('#sub').click(function(event) {
		...
		$.post('/Back/Product/visitendadd', {sprovince:sprovince,scity:scity,eprovince:eprovince,ecity:ecity,ecounty:ecounty,sm_platelets_price:sm_platelets_price,sm_driver_price:sm_driver_price,sm_code:sm_code}, function(data) {
			if(data['status'] == 'Y'){
                $.anewAlert(data['info']);
    ...
    $('#btn').click(function(event) {
        ...
        window.location.href="/Back/Product/visitend/provincea/"+provincea+"/city/"+city+"/county/"+county;
    });
	//获取添加起始市
    ...
    $('.see4').click(function(event) {
        ...
        if (confirm('确定删除此信息吗？')) {
            $.post('/Back/Product/visitenddel', {code: code}, function(data) {
                alert(data);
                window.location.reload();
            },'json');
        }
