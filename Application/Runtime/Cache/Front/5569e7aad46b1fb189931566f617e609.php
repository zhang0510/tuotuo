...
<div class="dengm">
 <div class="deng0  container">
   ...
   <form method="post" action="/Front/Order/normalOrderT" id="quickz">
   <div class="deng2" style="padding:0px;">
     <div class="deng2b">
	   <div class="deng2b1">
		<dl><dt><p class="bm1"><b>运输费</b></p></dt></dl>
		<dl>
			<dt>出发地：</dt>
			<dd>
				<input type="text" class="txt1a inko" readonly="readonly" name="qsrcz" id="qsrcz" value="<?php echo ($order["qsrezname"]); ?>" >
				<span class="xxsj"></span>
				<div class="selected" id="q4">
					...
					<div class="city star_city" data-id="" " id="qscz"></div>
					<div class="county star_county" data-id=""  id="qsbz"></div>
				</div> 
			</dd>
		</dl>
		<dl>
			<dt>目的地：</dt>
			<dd>
				<input type="text" class="txt1a inko" readonly="readonly" name="qercz" id="qercz" value="<?php echo ($order["qerezname"]); ?>" onclick="ceshi(this);">
				<span class="xxsj"></span>
				<div class="selected" id="q5">
		...
		<dl>
			<dt>品牌车型：</dt>
			<dd>
				<input type="text" class="txt1a inko" readonly="readonly" id="qbtz" name="carInfoz" value="<?php echo ($order["qsctez"]); ?>-<?php echo ($order["qsctcz"]); ?>" >
				<span class="xxsj"></span>
				<div class="selected" id="q6">
					<div class="province star_province" id="qbrz">
					<?php if(is_array($brand)): foreach($brand as $key=>$vo): ?><li data-id="1"  onclick="getTypez('<?php echo ($vo["cxjj_id"]); ?>','<?php echo ($vo["cxjj_brand"]); ?>')"><?php echo ($vo["cxjj_brand"]); ?></li><?php endforeach; endif; ?>
					</div>
					<div class="city star_city" data-id="" style="display:none" id="qtz"></div>
				</div> 
			</dd>
		</dl>
		<dl><dt>运费：</dt><dd><b id="totelz"><?php echo ($order["totalz"]); ?></b>元</dd></dl>
		<dl><dt><p class="bm1"><b>保险费</b></p></dt></dl>
		<dl><dt>保额：</dt><dd><input type="text" class="txt1" onblur="baojia();" onkeyup="if(event.keyCode !=37 && event.keyCode != 39){if (!/^[\d]+$/ig.test(this.value)){this.value='';}}" id="car_baojia"><font style="color:red;">(*请输入车辆当前价值单位:万)</font></dd></dl>
		<dl><dt>保险费：</dt><dd><b id="baoprice">0</b>元</dd></dl>
		<dl><dt>合计费用：</dt><dd><b id="heji"><?php echo ($order["totalz"]); ?></b>元</dd></dl>
		<!-- 开始地 -->
		<input type="hidden" name="qsrez" id="qsrez" value="<?php echo ($order["qsrez"]); ?>" />
		<!-- 目的地 -->
		<input type="hidden" name="qerez" id="qerez" value="<?php echo ($order["qerez"]); ?>" />
		<!-- 开始地中文名称 -->
		<input type="hidden" name="qsrezname" id="qsrezname" value="<?php echo ($order["qsrezname"]); ?>" />
		<!-- 目的地中文名称 -->
		<input type="hidden" name="qerezname" id="qerezname" value="<?php echo ($order["qerezname"]); ?>" />
		<!-- 车品牌 -->
		<input type="hidden" name="qsctez" id="qsctez" value="<?php echo ($order["qsctez"]); ?>" />
		<!-- 车型号 -->
		<input type="hidden" name="qsctcz" id="qsctcz" value="<?php echo ($order["qsctcz"]); ?>" />
		<!-- 车id -->
		<input type="hidden" name="carids" id="carids" value="<?php echo ($order["carids"]); ?>" />
		<!-- 运费 -->
		<input type="hidden" name="lineprice" id="lineprice" value="<?php echo ($order["lineprice"]); ?>" />
		<!-- 类型 -->
		<input type="hidden" name="product_type" id="product_type" value="<?php echo ($order["product_type"]); ?>" />
		<!-- 总费 -->
		<input type="hidden" name="totalz" id="totalz" value="<?php echo ($order["totalz"]); ?>" />
		<!-- 提车费 -->
		<input type="hidden" name="tiz" id="tiz" value="<?php echo ($order["tiz"]); ?>" />
		<!-- 送车费 -->
		<input type="hidden" name="songz" id="songz" value="<?php echo ($order["songz"]); ?>" />
		<!-- 集散地起点 -->
		...
		<!-- 毛利 -->
		<input type="hidden" name="maoliz" id="maoliz" value="<?php echo ($order["maoliz"]); ?>" />
		<!-- 普通下单钱数第一部分 -->
		...
		<!-- 数据是否可用 -->
		<input type="hidden" name="flag" id="flag" value='<?php echo ((isset($order["flag"]) && ($order["flag"] !== ""))?($order["flag"]):"N"); ?>' />
		<input type="hidden" name="msg" id="msg" value='' />
		<!-- 车价 -->
		<input type="hidden" name="order_info_price" id="order_info_price" value=0 />
		<!-- 保价 -->
		<input type="hidden" name="order_info_bxd" id="order_info_bxd" value='' />
		<div class="txt4"><input type="button" onclick="queFun();" value="确定"></div>
	 </div>
	   <div class="deng2b2">
...
//下一步
function queFun(){
	var baoprice = $('#baoprice').html();
	var car_baojia = $("#car_baojia").val();
	var flag = $("#flag").val();
	var msg = $("#msg").val();
	if(car_baojia.length >5){
		$('#baoprice').html(0);
		layer.msg("保价最多输入5位");
		return false;
	}
	...
	if(baoprice==0 || car_baojia==0 || car_baojia==""){
		layer.msg("请先输入保额");
		return false;
	}
...
function baojia(){
	...
	$.post("/Front/Order/getAcale",{car_baojia:car_baojia},function(data){
		$("#baoprice").html(data);
		$("#order_info_price").val(car_baojia);
		$("#order_info_bxd").val(data);
		totelFun();
	},'json');
}
function ceshi(obj){
