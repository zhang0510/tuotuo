...
<!--content-->
  <div class="banner0">
  <img src="/Public/Front/images/banner8.jpg">
  </div>
<div class="new_main">
<div class="new_body container">
    ...
    <div class="tnew12">
	<img src="<?php echo ($adv["adv_img"]); ?>">
	  <div class="tnew12a">
		  <div class="tnew12b">
			  <div class="tnew12c">
				<h2><?php echo ($adv["adv_img_title"]); ?></h2>
				<span></span>
			    <p><?php echo ($adv["adv_img_info"]); ?></p>
			  </div>
		  </div>
  	  </div>
  </div>
  <div class="pnew2">
   <?php if(is_array($list["list"])): foreach($list["list"] as $key=>$vo): ?><div class="col-sm-6 col-md-4 pnem1">
	   	<a href="javascript:;" onclick="autoCarFun('<?php echo ($vo["line_code"]); ?>');">
		   <div class="pnew2a"><img src="<?php echo ($vo["line_img"]); ?>"></div>
		   <div class="pe4a">
			 	<div class="pnew4a">
					<h2><?php echo ($vo["line_name"]); ?></h2>
					<span></span>
		    		<p><?php echo ($vo["start_city"]); ?>--><?php echo ($vo["end_city"]); ?></p>
			 	</div>
			 	<div class="pnew3a">
					<h2>￥<?php echo ($vo['price']['fuc']-$vo['line_discount']/100); ?>起</h2>
		    		<p>原价￥<?php echo ($vo['price']['fuc']); ?></p>
				</div>
		 	</div>
		 </a>
...
<script>
	function autoCarFun(code){
		$.post("/Front/Worklwt/checkLogins",{},function(data){
			if(data.flag){
				window.location.href='/Front/Worklwt/lineDis/code/'+code;
			}else{
				$.anewAlert(data.msg,0,1,'/Front/Login/pclogin');
			}
