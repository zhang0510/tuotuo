...
<script>
	...
	function ajaxstatus(){
		...
		//var wxpay = '<?php echo ($wxpay); ?>';
		$.post('/Front/Payment/checkStatus',{'code':code},function(data){
			if(data['pay_status']=='Y'&&data['wxpay']['sign']=='S'){
				//支付成功
				window.location="/Front/Payment/wxRes/res/s";
			}else if(data['pay_status']=='Y'&&data['wxpay']['sign']=='E1'){
				//支付成功，数据处理出现问题
				window.location="/Front/Payment/wxRes/res/e1";

			}else if(data['pay_status']=='Y'&&data['wxpay']['sign']=='E2'){
				//重复支付
				window.location="/Front/Payment/wxRes/res/e2";
			}
		});
	}
...
<img  style="margin: 0px auto 30px;display:block;" src="/Front/Payment/makeQRImgTwo"/>
-->
</div>
