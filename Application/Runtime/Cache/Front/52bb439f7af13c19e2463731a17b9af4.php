...
<!-- 页面自带JS开始-->
  ...
  $("#addLink").click(function(){
		...
		if(username != "" && tel != "" && idNum != ""){
			$.post("/Front/Personal/linkAdd",{"username":username,"tel":tel,"idNum":idNum},function(data){
				$.anewAlert(data,0,1,'/Front/Personal/linkMan');
			},'json')
				$("#username").html("");
				$("#call").html("");
				$("#idNum").html("");
	...
	function delLink(){
		var selid = '';
		  $(".del:checked").each(function(){
			...
			$.post("/Front/Personal/delLink",{"code":selid},function(data){
				$.anewAlert(data,0,1,'/Front/Personal/linkMan');
			},'json')
