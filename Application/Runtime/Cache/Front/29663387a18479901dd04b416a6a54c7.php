...
<div class="order_main container">
  ...
  <div class="order_detail">
	...
	<form  method="post" action="/Front/Ordersel/waybill">
		<div class="du3d">
		<?php if($status == 2): ?><input type="text" placeholder="请在此输入订单号" class="cx1" name="keyword" onkeyup="sel(this);" id="keyword" value="<?php echo ($keyword); ?>">
		 <?php else: ?>
			<input type="text" placeholder="请在此输入订单号" class="cx1" name="keyword" onkeyup="sel(this);" id="keyword"><?php endif; ?>
		<button type="submit"></button>
			<div id="searchBoxs" ></div>
		</div>
	</form>
	<?php if($status == 2): ?><div class="du3t">
	<?php if($wayList[0]['time'] != ''): ?><h2>订单跟踪</h2>
	  </div>
	<?php else: ?>
		<h2>您好，暂时还没有物流信息哦</h2>
	   </div><?php endif; ?>
		 ...
		 <div class="llki">
		  <div class="llki0"></div>
		<?php if(is_array($wayList)): foreach($wayList as $k=>$vo): ?><dl>
				 <dd>
					<p><?php echo ($vo["time"]); ?>   　　　 <?php echo ($vo["content"]); ?></p>
				 </dd>
			</dl><?php endforeach; endif; ?>
		   <!--<dl><dd>
			 <p>2016-07-18       周一    21:35:39     商家正通知快递公司揽件</p>
			 <p>2016-07-18       周一    21:35:39     商家正通知快递公司揽件</p>
			 </dd>
		  </dl>
		   <dl><dd>
	 ...
	 var inputText= $(obj).val();
		 $.post("/Front/Ordersel/selList",{keyword:inputText},function(data){
		 var size = data.length;
							//alert(size);
								if(size > 0){
									...
									var list="<ul>";
									for(i=0;i<size;i++){
										list+="<li class='see' >"+data[i]['order_code']+"</li>";
									}
									list+="</ul>";
									//alert(list);
									$("#searchBoxs").append(list);
									//alert(123);
							$(".see").click(function(){
