...
<div class="part1" style="width: 100%">
	...
	<div class="deng0">
		<div class="deng2">
			<div class="deng2a">
				<p><a  class="ww1" href="/Front/Login/pclogin">普通登录方式</a><a  class="ww2 on" href="javascript:void(0);">手机动态码登录</a></p>
			</div>
				...
				<div class="deng2b1">
					...
					<dl>
						...
						<dd>
							...
							<span id="captcha-container" class="txt3"><img style="height:46px" src="<?php echo U('Login/verify_c',array());?>" title="点击刷新" onclick="verify_img()"></span>
						</dd>
					</dl>
					<dl>
						<dt>验证码：</dt>
					...
					<div class="txt4"><input type="button" value="登录" onclick="Login_Check()"><a href="/Front/Register/index">注册</a></div>
...
function verify_img(){ 
	...
	captcha_img.attr("src", '/Front/Login/verify_c?random='+Math.random());
};
...
function checkCode(){
		...
		if(VCode!=""){
			$.post('/Front/Login/verifyCode',{'VCode':VCode},function(data){
				if(data){
					$.anewAlert(data);
				}
			})
...
function checkTel(){
		...
		if(tel!=""){
			if(!$.check_Mobile(tel)){
				...
				$.post('/Front/Login/verifyPhone',{'tel':tel},function(data){
					if(data<=0){
...
function SM_Check(){
	...
	if(tel!=""){
		if(!$.check_Mobile(tel)){
			...
			$.post('/Front/Login/verifyPhone',{'tel':tel},function(data_tel){
				if(data_tel<=0){
					$.anewAlert("对不起 你还不是注册用户");
					return false;
				}else{
					if(VCode!=""){
						$.post('/Front/Login/verifyCode',{'VCode':VCode},function(data_VCode){
							if(data_VCode==0){
								$.anewAlert("亲，验证码输错了哦！");
								return false;
							}else{
								$.post('/Front/Login/tel_SMS',{'tel':tel,"VCode":VCode},function(data){
									if(data.massion){
										//手机动态验证码等待60秒
...
function Login_Check(){
	...
	//提交
	$.post('/Front/Login/mobile_Login_Verify',{'tel':tel,'VCode':VCode,'SM_VCode':SM_VCode,'SMS_VCode':SMS_VCode},function(data){
		if(data.type==0){
			$.anewAlert(data.info);
		}else{
