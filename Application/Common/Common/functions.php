 ...
 */
  ...
  * @param number $mark   返回数据拼接（自定义）方法名getStr
  * @$data  扩展参数<可以是数组也可以是单个参数>
  * @compare 对比数组 用户确定子类是否是选定状态
  * @return $data  Array 返回值，可扩展
  */
 function recursionEx($id=0,$pidn="",$idn="",$data,$compare) {
     $list = array();
     foreach($data as &$v) {
         if($compare != ''){
             if(in_array($v['id'], $compare)){
                 $v['che'] = 'checked';
             }else{
                 $v['che'] = '';
             }
         ...
         if($v[$pidn] == $id) {
             $v['son'] = recursionEx($v[$idn],$pidn,$idn,$data,$compare);
             if(empty($v['son'])) {
                 unset($v['son']);
             }
             array_push($list, $v);
...
function send_notify_all(){
	$client = new \JPush\Client("57e53e9b3d19e550ca3f9e0a","6680d72a9d84687d446f9909",'./jpush.log');
	$result = $client->push()
	->setPlatform('all')
	->addAllAudience()
	->setNotificationAlert('发送消息------------>测试123')
	->send();
	print_log("---:".json_encode($result));
 ...
 */
     ...
     * 根据城市 获取iD
     * */
     function get_areaid($name){
         if(empty($name)){
             return "";
         }
         $area=M("area");
         $map['area_name']=array("eq",$name);
         $res=$area -> where($map) ->field("area_id") -> find();
         if($res){
             return $res['area_id'];
         }else{
             return "";
     ...
     function get_areaname($id){
         ...
         $res=$area -> where($map) ->field("area_name") -> find();
         if($res){
