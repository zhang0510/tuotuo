...
class WorklwtController extends BaseController {
    ...
    function daochu(){
        ...
        $country =I('country');
        if($country !=''){
            $where['area_id']= array('in' ,array($pro,$cval,$country));
            $ret = M("area")->where($where)->select();
        }else if($cval !='' && $country ==''){
            $map['_string'] = "area_id =$pro or area_id =$cval or area_pid =$cval";
            $ret = M("area")->where($map)->select();
        }else if($cval ==''){
            $ret = M("area")->select();
        }
